...
class SimulationApp:
    ...
    def generate_initial_boid_positions(self): #Generacja losowe początkowe połozenia dla boidów w symulacji 
        return [(random.randint(50, 750), random.randint(50, 550)) for _ in range(len(self.boids))]
    
    def generate_initial_hunter_positions(self): #Generacja losowe początkowe połozenia dla Huntera w symulacji 
        return [(random.randint(50, 750), random.randint(50, 550)) for _ in range(len(self.hunters))]
    ...
    def start_simulation(self): #Start symulacji
        ...
        initial_boid_positions = self.generate_initial_boid_positions() #Generacja nowych początkowych pozycje Boidów
        self.initial_boids_info = [(x, y, random.uniform(0, 360), random.uniform(1, 3)) for x, y in initial_boid_positions] #Zapisuje informacje o poczatkowych pozycjach Boidach
        initial_hunter_positions = self.generate_initial_hunter_positions() #Generacja nowych początkowych pozycje Boidów
        self.initial_hunters_info = [(x, y, random.uniform(0, 360), random.uniform(1, 3)) for x, y in initial_hunter_positions] #Zapisuje informacje o poczatkowych pozycjach Boidach
    ...
    def restart_simulation(self): #Reset symulacji
        ...
        for info in self.initial_boids_info:
            ...
            boid = Boid(self.canvas, x, y, int(self.boid_sensing_radius.get()))
        ...
        for info in self.initial_hunters_info:
            x, y, direction, speed = info
            hunter = Hunter(self.canvas, x, y, int(self.hunter_sensing_radius.get()))
        ...
        #Generacja nowych początkowych pozycji boidów
        initial_boid_positions = self.generate_initial_boid_positions()
        self.initial_boids_info = [(x, y, random.uniform(0, 360), random.uniform(1, 3)) for x, y in initial_boid_positions]

        #Generacja nowych początkowych pozycji bhunterow
        initial_hunter_positions = self.generate_initial_hunter_positions()
        self.initial_hunters_info = [(x, y, random.uniform(0, 360), random.uniform(1, 3)) for x, y in initial_hunter_positions]
